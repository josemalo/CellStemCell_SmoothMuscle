...
##############################################################################
### Modified version by Jose Malagon-Lopez for the AI lab at WUXI NextCode ###
############################### 2019-01-29 ###################################
##############################################################################
...
#---------------------------------------------------------------
	...
	# ------------------------------------------------------------------------------------------ #
	########################################## New Code ##########################################
	def performVAEclustering(self):
		print('start clustering...')
		KET = self.KET   # sorted time points
		dCK = self.getdCK(self.kc)
		# pdb.set_trace()
		AC = []
		gc.collect()
		for i in range(len(KET)):
			print("collecting clustering for time: " + str(KET[i]))
			ti = KET[i]         # current time point
			CT = self.dET[ti]   # cells at current time point
			CKT = dCK[ti]       # number of clusters at current time point
...
class Graph:
	def __init__(self,Cells,tfdna,kc,largeType=None,dsync=None,virtualAncestor=None,fChangeCut=1.0,etfile=None):
		self.Cells=Cells
	...
	########################################## Modified ##########################################
	def buildNodes(self):
		return(self.clustering.performVAEclustering())
	########################################## ######## ##########################################
	# ------------------------------------------------------------------------------------------ #
